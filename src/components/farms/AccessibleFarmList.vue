  ...
  <div class="w-full bg-blue-600">
    ...
    <div ref="filterSection" class="bg-white border-b border-gray-200">
      <div class="p-8 max-w-7xl mx-auto">
        <div class="flex gap-4 items-end flex-wrap">
          ...
          <!-- 無障礙設施篩選 -->
          <div class="flex-1 min-w-[180px]">
            <label for="accessibleItemSelect" class="block font-semibold text-gray-700 mb-2"
              >無障礙設施</label
            >
            <select
    ...
    >
      <div class="p-8 max-w-7xl mx-auto">
        <h5>📊 統計資訊</h5>
        <p>
          共找到 <strong>{{ filteredData.length }}</strong> 筆無障礙休閒農場資料（目前顯示第
        ...
        <div v-if="selectedCounty || selectedAccessibleItem" class="text-sm mt-2">
          ...
          >
          <span
            v-if="selectedAccessibleItem"
            class="inline-block bg-blue-200 px-2 py-1 rounded text-xs"
            >♿ {{ selectedAccessibleItem }}</span
          >
        </div>
      </div>
    </div>
