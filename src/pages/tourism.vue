<template>
  <div class="min-h-screen bg-blue-600 w-full">
    ...
    <div v-if="selectedCategory === 'farms'" class="flex min-h-[calc(100vh-200px)]">
      ...
      >
        <div class="mb-8 text-center border-b-2 border-gray-200 pb-2">
          <h3
      ...
      <div class="flex-1 bg-gray-50 overflow-y-auto">
        <!-- 無障礙農場組件 -->
        <AccessibleFarmList v-if="selectedFarmType === 'accessible'" />

        <!-- 戶外教育農場組件 -->
        <OutdoorEduFarmList v-else-if="selectedFarmType === 'outdoor-edu'" />
      </div>
    </div>
    ...
    <!-- 市集分類的內容區域 -->
    <div v-else-if="selectedCategory === 'markets'" class="w-full bg-gray-50 overflow-y-auto">
      <MarketList />
    </div>
    ...
    <div v-else-if="selectedCategory === 'souvenirs'" class="flex min-h-[calc(100vh-200px)]">
      ...
      <div class="flex-1 bg-gray-50 overflow-y-auto">
        ...
        <!-- 農漁會年度百大農業精品好禮組件 -->
        <AgriBestItemList v-else-if="selectedSouvenirType === 'rural-goods'" />

        <!-- 農民學院找產品組件 -->
        <MemberProductList v-else-if="selectedSouvenirType === 'member-product'" />
      </div>
    </div>
